...
class LARRecourse:
    ...
    def direction(self, weights: np.ndarray):
        return np.sign(weights)

    def sign(self, x, direction):
        s = np.sign(x)
    ...
    def get_robust_recourse(self, x_0: np.ndarray):
        ...
        directions = self.direction(weights)
        while True:
            if all(changed):
                break
            i = self.get_max_idx(weights, changed)
            c = np.matmul(x, weights) + bias
            delta = self.calc_delta(weights[i], c[0])
            if self.sign(x[i] + delta, directions[i]) == self.sign(x[i], directions[i]):
                x[i] += delta
                break
            else:
                x[i] = 0
                if np.abs(self.weights[i]) > self.alpha:
                    weights[i] = weights[i] + (self.alpha * np.sign(x_0[i]))
                else:
                    changed[i] = True
        return x
